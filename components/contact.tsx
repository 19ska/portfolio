...
export function Contact() {
  return (
    <section id="contact" className="relative overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 bottom-0 h-[30rem] bg-[radial-gradient(60rem_28rem_at_50%_110%,rgba(79,124,255,0.22),transparent_65%)]"
      />
        ...
        <Reveal stagger className="mx-auto max-w-3xl text-center">
          ...
          <RevealItem>
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-ink sm:text-5xl lg:text-[64px] lg:leading-[1.02]">
              Let&apos;s build <span className="text-glow text-accent-strong">something.</span>
            </h2>
          </RevealItem>
          <RevealItem>
              ...
              {links.map(({ label, value, href, Icon, external }) => (
                <li key={label}>
                  <a
                    ...
                    className="glass group flex items-center gap-4 rounded-xl p-4 text-left transition-colors hover:border-accent/60"
                  >
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-accent-strong transition-colors group-hover:bg-accent group-hover:text-bg">
                    ...
                    <ArrowUpRight className="ml-auto h-4 w-4 shrink-0 text-faint transition-colors group-hover:text-accent-strong" />
                  </a>
                </li>
              ))}
            </ul>
